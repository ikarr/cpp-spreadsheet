...
void Sheet::SetCell(Position pos, std::string text) {
    ...
    // проверка существования ячейки с данным индексом и значением
    ...
    // создание временного объекта-ячейки
    ...
    // добавление созданной ячейки на лист
...
void Sheet::CalculateSize_(Position&& erased_pos) {
    ...
    std::for_each(sheet_.begin(), sheet_.end(), [&new_size, grater_col_idx](const auto& row) {
        ...
        new_size.cols = !grater_col_idx
            ? new_size.cols
            : std::max(std::max_element(row.second.begin(), row.second.end(),
